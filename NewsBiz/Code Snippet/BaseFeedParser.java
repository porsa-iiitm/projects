...
public class BaseFeedParser {
	...
	static String feedUrlString = null;
	...
	public List<Message> parse() {
		...
		item.getChild(MEDIA_THUMBNAIL).setEndTextElementListener(
				new EndTextElementListener() {
					public void end(String body) {
						currentMessage.setMediaThumbnail(body);
					}
				});
		...
		try {
			if (Splash.encoding = true) {
				Xml.parse(this.getInputStream(), Xml.Encoding.ISO_8859_1, root
						.getContentHandler());
			} else
